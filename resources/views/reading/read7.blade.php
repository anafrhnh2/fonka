...
@section('game-title', 'Tiga Suku Kata')
...
@section('content')
    ...
    <div id="game-fullscreen-container"
        ...
        <div class="flex flex-wrap justify-between items-center mb-4 shrink-0 w-full max-w-6xl mx-auto">
            ...
            <div class="flex items-center gap-3 font-lexend">
                <div class="flex flex-col items-center bg-white border-4 border-teal-200 rounded-2xl px-6 py-2 shadow-md">
                    <span class="text-teal-500 font-bold text-sm uppercase tracking-wider">Perkataan</span>
                    <span id="round-display" class="text-teal-700 font-black text-2xl">1/10</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 w-full max-w-5xl mx-auto flex flex-col items-center justify-center relative z-10">
            ...
            <div class="w-full bg-slate-200 rounded-full h-4 mb-6 overflow-hidden shadow-inner">
                <div id="progress-bar" class="bg-teal-500 h-4 rounded-full transition-all duration-500" style="width: 0%">
                </div>
            </div>
            ...
            <div id="flashcard"
                ...
                <button onclick="playExample()"
                    class="absolute top-6 left-6 w-20 h-20 bg-sky-100 text-sky-500 rounded-full flex items-center justify-center text-2xl border-4 border-sky-200 hover:bg-sky-200 active:scale-95 transition-all z-30 shadow-sm"
                    title="Eja dan Sebut">
                ...
                <div class="flex flex-col items-center justify-center mt-4 w-full">
                    <h2 id="spelling-display"
                        class="text-2xl md:text-3xl font-bold text-slate-400 font-lexend tracking-[0.2em] uppercase mb-2">KE
                        - RE - TA</h2>

                    <div
                        class="bg-teal-50 border-4 border-teal-200 w-full max-w-sm rounded-[2rem] py-2 flex flex-row items-center justify-center shadow-inner gap-1 md:gap-2">
                        <h1 id="suku1-display"
                            class="text-4xl md:text-5xl font-black text-rose-500 font-lexend capitalize tracking-wide">Ke
                        </h1>
                        <h1 id="suku2-display"
                            class="text-4xl md:text-5xl font-black text-sky-500 font-lexend lowercase tracking-wide">re</h1>
                        <h1 id="suku3-display"
                            class="text-4xl md:text-5xl font-black text-amber-500 font-lexend lowercase tracking-wide">ta
                        </h1>
                    </div>
                </div>
                ...
                <button id="btn-mic" onclick="toggleListening()"
                    class="mt-2 w-20 h-20 bg-teal-500 text-white rounded-full flex items-center justify-center text-4xl border-b-[6px] border-teal-700 active:border-b-0 active:translate-y-2 transition-all shadow-[0_15px_30px_rgba(20,184,166,0.4)] relative z-20">
    ...
    <script>
        ...
        const suku3Display = document.getElementById('suku3-display');
        ...
        function setupSpeechRecognition() {
            ...
            recognition.onstart = function() {
                ...
                statusText.className =
                    "text-red-500 font-bold text-xl md:text-2xl text-center mt-2 mb-4 h-8 animate-pulse";
                transcriptDisplay.innerText = "Mendengar...";
                transcriptDisplay.className =
                    "text-amber-500 font-bold text-base md:text-lg italic animate-pulse text-center w-full";
            };
            ...
            recognition.onresult = function(event) {
                ...
                transcriptDisplay.className =
                    "text-slate-700 font-bold text-xl md:text-2xl text-center w-full truncate";
            ...
            recognition.onend = function() {
                if (isListening && !isProcessing) {
                    ...
                    transcriptDisplay.className =
                        "text-slate-400 font-bold text-base md:text-lg italic text-center w-full";
                    const currentWord = sukuKataData[currentIndex].word;
                    statusText.innerText = `Sebut perkataan: "${currentWord}"`;
                    statusText.className = "text-orange-500 font-bold text-xl md:text-2xl text-center mt-2 mb-4 h-8";
                }
                isListening = false;
        ...
        function toggleListening() {
            ...
            if (isListening) {
                ...
                try {
                    ...
                    statusText.className = "text-slate-600 font-bold text-xl md:text-2xl text-center mt-2 mb-4 h-8";
                    recognition.start();
                } catch (e) {
                    recognition.stop();
                }
        ...
        function loadCard() {
            ...
            suku3Display.innerText = data.suku3;

            spellingDisplay.innerText =
                `${data.suku1.toUpperCase()} - ${data.suku2.toUpperCase()} - ${data.suku3.toUpperCase()}`;
            ...
            transcriptDisplay.className = "text-slate-400 font-bold text-base md:text-lg italic text-center w-full";
        ...
        function checkPronunciation(spokenWord) {
            ...
            if (isCorrect) {
                ...
                statusText.className = "text-red-500 font-bold text-xl md:text-2xl text-center mt-2 mb-4 h-8";
        ...
        function playExample() {
            ...
            phonicAudio.play().catch(error => {
                ...
                playPhonicsTTS(`${data.suku1} ... ${data.suku2} ... ${data.suku3} ... ... ${data.word}`, 0.6);
            });
        }
